    ...
    <div className="min-h-screen bg-[#0b0904]">
      ...
      <div className="container max-w-6xl py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {statCards.map((s) => (
            <div key={s.label} className="bg-[#130f07] border border-white/6 rounded-2xl p-5 text-center">
        ...
        <Tabs defaultValue="users">
          ...
          <TabsContent value="users" className="mt-4">
            <div className="bg-[#130f07] border border-white/6 rounded-2xl overflow-hidden">
              <div className="px-6 py-4 border-b border-white/6">
                <h2 className="font-bold text-white">All Users ({usersData?.total ?? 0})</h2>
              </div>
              <div className="divide-y divide-white/5">
                {(usersData?.users ?? []).map((user) => (
                  <div key={user.id} className="flex items-center justify-between px-6 py-4 hover:bg-white/2 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-[#f5c518] text-black flex items-center justify-center font-black text-sm flex-shrink-0">
          ...
          <TabsContent value="jobs" className="mt-4">
            <div className="bg-[#130f07] border border-white/6 rounded-2xl overflow-hidden">
              <div className="px-6 py-4 border-b border-white/6">
                <h2 className="font-bold text-white">All Jobs ({jobsData?.total ?? 0})</h2>
              </div>
              <div className="divide-y divide-white/5">
                  ...
                  <Link href={`/jobs/${job.id}`} key={job.id}>
                    <div className="flex items-center justify-between px-6 py-4 hover:bg-white/2 cursor-pointer transition-colors group">
                      ...
                      <div className="flex items-center gap-2 ml-3">
                        <span className={`text-xs font-semibold px-2.5 py-1 rounded-md ${JOB_STATUS[job.status] ?? "bg-white/8 text-white/40"}`}>
